...
export default function LevelUpOverlay({ isOpen, onClose, userData, level, realm }) {
  if (!isOpen) return null;

  const totalExp = safeNum(userData?.profile?.totalExp, 150);
  const levelInfo = calculateLevel(totalExp);
  const currentLevel = level || levelInfo.level || 1;
  const rank = realm ? { name: realm.name, badge: realm.badge, color: realm.color } : getRankTier(currentLevel);
  const stats = userData?.profile?.stats || { int: 45, wis: 30, dex: 25, vit: 20 };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-xl animate-fade-in font-orbitron">
      <div className="cyber-panel max-w-md w-full rounded-3xl p-6 border-2 border-cyan-400 shadow-2xl relative text-center overflow-hidden animate-cyber-pulse cyber-hud-brackets bg-slate-950/95">
        ...
        <span className="text-xs font-black tracking-widest uppercase text-cyan-400 bg-cyan-950/90 px-3.5 py-1 rounded-full border border-cyan-500/60 shadow-lg shadow-cyan-500/30">
          LEVEL UP OVERCHARGE!
        </span>

        <h2 className="text-3xl font-black text-white mt-3 mb-1 tracking-wider">
          LEVEL {currentLevel}
        </h2>
        ...
        <div className="my-5 bg-slate-950/80 rounded-2xl p-4 border border-slate-800 text-left space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs font-semibold">
            <div className="flex justify-between p-2 rounded-lg bg-slate-900 border border-blue-500/30">
              ...
              <span className="text-white font-mono">{safeNum(stats.int, 45)}</span>
            </div>
            <div className="flex justify-between p-2 rounded-lg bg-slate-900 border border-purple-500/30">
              ...
              <span className="text-white font-mono">{safeNum(stats.wis, 30)}</span>
            </div>
            <div className="flex justify-between p-2 rounded-lg bg-slate-900 border border-emerald-500/30">
              ...
              <span className="text-white font-mono">{safeNum(stats.dex, 25)}</span>
            </div>
            <div className="flex justify-between p-2 rounded-lg bg-slate-900 border border-amber-500/30">
              ...
              <span className="text-white font-mono">{safeNum(stats.vit, 20)}</span>
            </div>
          </div>
        </div>
        ...
        <button
          onClick={onClose}
          className="w-full py-3.5 rounded-xl bg-gradient-to-r from-cyan-400 via-purple-500 to-pink-500 hover:from-cyan-300 hover:to-pink-400 text-slate-950 font-black text-xs tracking-widest uppercase shadow-xl shadow-cyan-500/30 transition active:scale-95 flex items-center justify-center gap-2"
        >
